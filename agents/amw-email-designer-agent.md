...
| Email-specific patterns (table-layout single-column, nested-table Outlook conditionals, prefers-color-scheme + Outlook mobile `meta color-scheme` dark-mode, plain-text fallback structural mirror, transactional vs marketing voice + compliance, image-blocked rendering, preheader padding) | [TECH-email-design-patterns](../skills/amw-design-principles/references/TECH-email-design-patterns.md) | Email render-surface invariants (no JS, no flexbox, no SVG, 600px max, inlined styles); covers thematic email items in T-111..T-130 |
...
- **Run bin scripts directly for mechanical operations** via Bash:
  ```
  Bash: bash bin/amw-mjml-render.sh --render input.mjml output.html
  ```
