...
5. **Design the easing vocabulary.** If `brand_tokens.easing.brand_curve` is provided, use it as the primary easing. Otherwise consult the canonical [TECH-motion-easing-catalog](../skills/amw-design-principles/references/TECH-motion-easing-catalog.md) — six Material Design 3 curves (`--ease-standard`, `--ease-standard-decelerate`, `--ease-standard-accelerate`, `--ease-emphasized`, `--ease-emphasized-decelerate`, `--ease-emphasized-accelerate`) mapped onto the 7 roles from `TECH-motion-taxonomy.md`. Above-the-fold entries get `--ease-emphasized-decelerate`; routine entries get `--ease-standard-decelerate`; exits mirror with the accelerate variants; page transitions use a split in/out curve. Cap emphasized easings to one above-the-fold motion per viewport — overuse drains the emphasis.

5a. **Apply perceptual duration tiers.** Pick the role's duration band from `TECH-motion-budgets.md`, then validate the chosen value against the four perceptual tiers in [TECH-motion-duration-budgets](../skills/amw-design-principles/references/TECH-motion-duration-budgets.md) — Micro (<100 ms), Short (100–300 ms), Medium (300–500 ms), Long (500–1000 ms). Anything exceeding 1000 ms is loading-state territory and MUST be wrapped in a skeleton screen / spinner / progress UI rather than animated raw. Hover/focus durations belong in Micro/Short; modal entrances in Medium; page transitions at the Medium/Long boundary.

5b. **Pick an orchestration pattern + audit the viewport budget.** For multi-element motion, consult [TECH-motion-orchestration](../skills/amw-design-principles/references/TECH-motion-orchestration.md) and pick exactly one of cascade / sequence / parallel / interrupted. Cascade stagger window MUST cap at 600 ms total; sequences cap at three steps. Then audit against [TECH-motion-perceptual-budgets](../skills/amw-design-principles/references/TECH-motion-perceptual-budgets.md): ≤5 simultaneous animated regions per viewport, ≤3 distinct easings concurrently, exactly 1 ambient slot. If the proposed set exceeds the cap, add a `warnings` entry and propose which animations to defer (ambient first, then non-hero entries). The `prefers-reduced-motion: reduce` block collapses all durations to 1 ms while preserving the state change — this is mandatory, not optional.
...
| Always — easing curve selection (per role) | [TECH-motion-easing-catalog](../skills/amw-design-principles/references/TECH-motion-easing-catalog.md) | Six MD3 named curves (`--ease-standard`, `--ease-emphasized-decelerate`, etc.) + role mapping; covers T-107 |
| Named microinteractions catalog (hover-lift / focus-ring / press-shrink / success-pulse / error-shake / loading-shimmer / goal-gradient-pulse / native-popover-open / Popmotion-spring) + GSAP `useGSAP` rules (T-111) + creative-coder purpose→spec→impl→a11y→perf→next checklist (T-112) | [TECH-microinteractions-catalog](../skills/amw-design-principles/references/TECH-microinteractions-catalog.md) | Token form + reduced-motion fallback per entry; T-111..T-114, T-123 fragment |
> [TECH-motion-easing-catalog.md] Token block · Role mapping (entry / exit / hover / focus / interaction / page-transition / ambient) · Per-curve detail · Aliases vs the MD3 catalog · Code samples (CSS + Web Animations API) · Breaks if · See also
| Always — duration tier validation | [TECH-motion-duration-budgets](../skills/amw-design-principles/references/TECH-motion-duration-budgets.md) | Four perceptual tiers (Micro <100 ms / Short / Medium / Long 1000 ms) + `--duration-quick/fast/short/medium/long` tokens; covers T-108 |
> [TECH-motion-duration-budgets.md] Token block · Four perceptual tiers · Boundary rationale (Doherty Threshold, RAIL) · Mapping to existing role budgets · Code samples · Breaks if · See also
| Multi-element motion — orchestration pattern | [TECH-motion-orchestration](../skills/amw-design-principles/references/TECH-motion-orchestration.md) | Pick exactly one of cascade / sequence / parallel / interrupted; CSS + Web Animations API recipes; covers T-109 |
> [TECH-motion-orchestration.md] Token block · The four orchestration patterns · Cascade (≤600 ms total window) · Sequence (≤3 steps) · Parallel (eye reads as one unit) · Interrupted (cancel + reverse) · Pattern decision tree · Breaks if · See also
| Always — viewport-level budget audit | [TECH-motion-perceptual-budgets](../skills/amw-design-principles/references/TECH-motion-perceptual-budgets.md) | ≤5 concurrent regions, ≤3 concurrent easings, exactly 1 ambient slot; mandatory reduced-motion override; covers T-110 |
> [TECH-motion-perceptual-budgets.md] Token block · Four hard rules · Concurrent region count · Distinct easing count · prefers-reduced-motion override · Interactive vs ambient footprint · Code samples · Runtime debug auditor · Breaks if · See also
