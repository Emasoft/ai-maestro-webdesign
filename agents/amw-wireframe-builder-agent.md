...
- Layout discipline (mechanical counts): [TECH-layout-discipline](../skills/amw-design-principles/references/TECH-layout-discipline.md) — hero-fit / hero-stack ≤ 4 / nav single-line ≤ 80px / bento exact-cell-count / eyebrow ≤ ceil(sections/3) / zig-zag ≤ 2 consecutive / ≥ 4 layout-families per 8 sections. I run these counts on the emitted HTML alongside the ai-slop gate; a failing count is a `warnings` entry (or `status=partial` if it means the ASCII-declared structure cannot render cleanly).
> [TECH-layout-discipline.md] What this is · Hero discipline · Navigation discipline · Section-layout discipline · Bento / grid discipline · Logo wall discipline · List discipline · Mobile collapse · The mechanical pre-flight counts · Cross-references
...
17. **Run slop-verifier gate (always — last step before delivery).**
    ...
    3. Dispatch `amw-slop-verifier-agent` (spec: [amw-slop-verifier-agent](../agents/amw-slop-verifier-agent.md)) with:
...
| Input contract carries `design_md_path` (a Variant 1 DESIGN.md is the canonical token source for this run) | [SKILL](../skills/amw-design-md/SKILL.md) + [TECH-15-design-md-as-input](../skills/amw-design-md-convert/references/TECH-15-design-md-as-input.md); lint via `bin/amw-design-md-lint.sh` BEFORE rendering any HTML | Treat DESIGN.md tokens as canonical (override `brand_tokens` if both supplied). On lint failure (P0/P1 errors), STOP — return `status=failed` with the lint error list in `blocking_issues` and `next_action=escalate_to_user`. Never render HTML against a broken DESIGN.md. |
