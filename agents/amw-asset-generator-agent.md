...
| "dashboard chart widget" + `target_stack=shadcn+next` or `shadcn+vite` | `skills/amw-shadcn-ui/docs/components/chart.mdx` (Recharts wrapper) + `skills/amw-svg-creator/` | TECH-chart-rendering (Recharts row) | shadcn-ui exposes a typed Recharts wrapper that respects design tokens. |
