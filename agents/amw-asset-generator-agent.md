...
| "dashboard chart widget" + `target_stack=shadcn+next` or `shadcn+vite` | `skills/amw-shadcn-ui/docs/components/radix/chart.mdx` (Recharts wrapper) + `skills/amw-svg-creator/` | TECH-chart-rendering (Recharts row) | shadcn-ui exposes a typed Recharts wrapper that respects design tokens. |
...
- **Do not invoke `design-principles` skill directly.** The orchestrator is upstream. If I need color system or typography rules, I read the specific reference files: `skills/amw-design-principles/color-system.md`, `skills/amw-design-principles/typography-system.md`, `skills/amw-design-principles/ai-slop-avoid.md`.
