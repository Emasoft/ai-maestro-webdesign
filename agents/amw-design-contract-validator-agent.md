...
- The BLOCK / FLAG / PASS severity model from design-forge's upstream contract-validator reference doc (MIT direct-port): BLOCK = at least one hard rule violated; FLAG = required fields present but advisory fields weak / empty / inconsistent; PASS = required fields present + advisory fields strong.
