...
| Form error recovery strategy (inline-first vs submit-summary vs progressive-disclosure vs server-replay) + announce-on-error a11y wiring + native `:has(:user-invalid)` / `inert` / `autocomplete` / `enterkeyhint` / Popover-API + fork-discipline boundary (T-118) + announce-plan cue (T-119) | [TECH-form-error-recovery](../skills/amw-design-principles/references/TECH-form-error-recovery.md) | Recovery flow + edge-state edges; complements R1 T-131 (blur timing) and T-132 (ARIA wiring) |
