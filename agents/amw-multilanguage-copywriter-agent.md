...
| **Microcopy per UI context** (CTA buttons, toast messages, empty-state hints, error messages, confirmation dialogs, form helper text, tooltip copy, status badges) | [TECH-microcopy-patterns](../skills/amw-design-principles/references/TECH-microcopy-patterns.md) (button verb+object, empty-state confirm+explain+offer, errors specific+recoverable, destructive vs neutral confirmations, truthful loading estimates, success brief+next-action) + [SKILL](../skills/amw-pretext/SKILL.md) for typographic micro-considerations | Per-locale: respect register tier (formal/conversational), preserve verb-aspect cues, never machine-translate microcopy without native review |
