...
8. **Confidence is tied to extraction *method*, and I enumerate what I could not determine.** (anydesign honesty discipline, uxKero/anydesign MIT.) Each emitted value's confidence follows HOW it was obtained, not how plausible it looks: a value read from a CSS custom property / Tailwind token / Figma variable is **high** (✅ — the authors named it); a value inferred from a computed style or a single observed instance is **medium** (⚠️); a value approximated from vision or a heuristic guess is **low** (❓). I surface every medium/low inference AND everything I could not determine at all in `recommendations[]` / `warnings[]` as explicit **Open Questions** — each naming the specific missing input and the human action that would resolve it (e.g. "primary-hover state not observed — provide a hover screenshot, or confirm it's a 10% darken"). This pairs with criterion 1: an unknown is *flagged*, never invented. (`_meta.fingerprint` records WHAT was extracted; this criterion records HOW sure I am of each part — and the open-questions live in the return contract, not as ad-hoc sections in the fixed DESIGN.md schema.)
...
4. **Fingerprint (`fingerprint: true`, automatic when any of the three flags above is set)** — computes SHA-256 over the canonicalized token block (colors / typography / spacing / rounded / borders / shadows / breakpoints / components / iconSystem) per [TECH-extractor-fingerprinting](../skills/amw-design-md-extract/references/TECH-extractor-fingerprinting.md). Writes to DESIGN.md `_meta.fingerprint` and `DESIGN.assets.json#fingerprint_hash`. Round-trip verification: re-parse the produced DESIGN.md, re-compute the hash, MUST match — mismatch is `status=failed` with `blocking_issues=["fingerprint round-trip mismatch — writer bug"]`.
...
| `extract_components: true` (codebase mode) | run five-pattern scanner; see [TECH-extractor-component-detection](../skills/amw-design-md-extract/references/TECH-extractor-component-detection.md) | T-091/T-094 — populate `## 4. Component Stylings` with variants + states from `cva()` / TS unions / prop interfaces / switch / object maps |
> [TECH-extractor-component-detection.md] When the extractor runs component detection · Codebase detection — five patterns, in priority order · URL detection — DOM landmarks + ARIA roles · State detection (T-094) · shadcn / Radix specific signals · What the extractor skips · Confidence and fallbacks · Output — what lands in DESIGN.md · Validation gate · Cross-references
...
> [TECH-extractor-icon-asset-export.md] When this runs · Phase A — Discovery · Phase B — Role assignment (T-093 + T-095) · Phase C — Alt-text inference (T-095, raster only) · Phase D — Export · License + provenance · Privacy + robots compliance · What the extractor refuses to do · Cross-references
| `fingerprint: true` (or any of `extract_*` flags) | compute SHA-256 over canonicalized token block; write to `_meta.fingerprint` + `DESIGN.assets.json#fingerprint_hash`; see [TECH-extractor-fingerprinting](../skills/amw-design-md-extract/references/TECH-extractor-fingerprinting.md) | T-096 — content-identity hash for cross-project DESIGN.md library, drift detection, "same design system, different content" recognition |
> [TECH-extractor-fingerprinting.md] Goal · Algorithm · Concrete recipe (pure-Python, no extra deps) · Interpretation guide · Non-goals (what fingerprinting is NOT) · Versioning the algorithm · Validation gate · Cross-references
| Always — DESIGN.md format spec | [SKILL](../skills/amw-design-md/SKILL.md) | Canonical Variant 1 structure and token contracts |
| Variant 1 spec details | [canonical-spec-google-alpha](../skills/amw-design-md-spec/references/canonical-spec-google-alpha.md) | Field-level spec for every YAML key |
...
| Validation failure recovery | [TECH-14-validation-failure-recovery](../skills/amw-design-md-audit/references/TECH-14-validation-failure-recovery.md) | What to do when lint fails persistently |
...
Action: extract to Variant 1 first (canonical). Note in `recommendations` that V1→V2 conversion is not yet implemented in `bin/`; the user can manually re-format the prose sections per [community-9-section-template](../skills/amw-design-md-spec/references/community-9-section-template.md).
...
- [TECH-14-validation-failure-recovery](../skills/amw-design-md-audit/references/TECH-14-validation-failure-recovery.md) — lint failure recovery
