...
5. **No agent file carries a `model:` key** — every agent, main-agent included, inherits the session model. For the 23 sub-agents this is required by role-plugins spec **RP-VAL-06**. For the main-agent it is our own call: **RP-MODEL-01** was CORRECTED 2026-08-08 (spec-version 1.0.1), which makes the main-agent pin an OPEN question owned by `TRDD-TYB3Q1NJ` and states in as many words that the clause **MUST NOT** be cited as requiring an `opus` pin. A pin here would override whatever model the dispatch site chose — the same harm that motivated stripping the sub-agent pins — and the bare `opus` token is movable (2.1.219 re-pointed it to Opus 5 at a different price). A caller that wants Opus passes it at `Agent()` dispatch time. Do not re-add pins; revisit when `TRDD-TYB3Q1NJ` closes.
...
|---|---|---|
| Opus 5 default + family-alias resolution (2.1.219, alias step-down fixed 2.1.222) | **adopted — content changed** | `sonnet` / `opus` are family aliases resolving to the *current* tier: today `opus` → Opus 5, `sonnet` → Sonnet 5. **No agent file pins a model any more.** The 23 `amw-*` sub-agents dropped `model: sonnet` per role-plugins spec **RP-VAL-06**; the main-agent then dropped `model: opus` too, and the advisory `model` key came out of `ai-maestro-webdesign.agent.toml` with it (only `name` / `compatible-titles` / `compatible-clients` are hard per **RP-TOML-MANDATORY**). Rationale in hard invariant 5 above — a pin overrides the operator's own model choice, and the bare `opus` token is movable. |
