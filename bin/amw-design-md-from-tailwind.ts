#!/usr/bin/env node
/**
 * amw-design-md-from-tailwind.ts — Pure-local Tailwind config + globals.css → DESIGN.md.
 *
 ...
 *
 * Usage:
 *   npx tsx bin/amw-design-md-from-tailwind.ts \
 ...
 *
 * Or (if compiled to JS / running with Node + ts-node):
 *   node bin/amw-design-md-from-tailwind.js --config ... --css ...
 */
...
async function loadTailwindConfig(configPath: string): Promise<TailwindTokens> {
  ...
  if (abs.endsWith(".ts") || abs.endsWith(".mts")) {
    ...
    const jiti = (jitiMod as any).createJiti
      ? (jitiMod as any).createJiti(import.meta.url)
      : (jitiMod as any).default(import.meta.url);
    rawConfig = jiti(abs) as Record<string, unknown>;
    if ("default" in rawConfig) rawConfig = rawConfig.default as Record<string, unknown>;
  } else {
    const req = createRequire(import.meta.url);
    rawConfig = req(abs) as Record<string, unknown>;
    if ("default" in rawConfig) rawConfig = rawConfig.default as Record<string, unknown>;
  }

  const theme = (rawConfig.theme as Record<string, unknown>) ?? {};
  const extend = (theme.extend as Record<string, unknown>) ?? {};

  return {
    colors: mergeDeep(
      (theme.colors as TailwindColors) ?? {},
      (extend.colors as TailwindColors) ?? {}
    ),
    borderRadius: {
      ...((theme.borderRadius as Record<string, string>) ?? {}),
      ...((extend.borderRadius as Record<string, string>) ?? {}),
    },
    spacing: {
      ...((theme.spacing as Record<string, string>) ?? {}),
      ...((extend.spacing as Record<string, string>) ?? {}),
    },
    fontSize: {
      ...((theme.fontSize as Record<string, unknown>) ?? {}),
      ...((extend.fontSize as Record<string, unknown>) ?? {}),
    },
    fontFamily: {
      ...((theme.fontFamily as Record<string, string | string[]>) ?? {}),
      ...((extend.fontFamily as Record<string, string | string[]>) ?? {}),
    },
  };
}

function mergeDeep(a: TailwindColors, b: TailwindColors): TailwindColors {
  const result = { ...a };
  for (const key of Object.keys(b)) {
    const av = result[key];
    const bv = b[key];
    if (typeof av === "object" && typeof bv === "object" && !Array.isArray(av)) {
      result[key] = mergeDeep(av as TailwindColors, bv as TailwindColors);
    } else {
      result[key] = bv;
    }
  }
...
function parseCssVariables(cssPath: string): CssVariables {
...
function extractFromBlock(css: string, selector: string): Record<string, string> {
...
function resolveColor(value: string, cssVars: Record<string, string>): string | null {
...
function flattenColors(
  ...
  for (const [key, val] of Object.entries(colors)) {
    ...
    if (typeof val === "string") {
      ...
      Object.assign(out, flattenColors(val as TailwindColors, cssVars, fullKey));
    }
  }
  return out;
}
...
function deriveComponents(colors: Record<string, string>): Record<string, Record<string, string>> {
...
function mapTokens(tailwind: TailwindTokens, cssVars: CssVariables): DesignTokens {
...
function generateDesignMd(tokens: DesignTokens, name: string, description: string): string {
  const lines: string[] = [];
  lines.push("---");
  ...
  // Colors section grouped by semantic role
  lines.push("## Colors");
  lines.push("");
  lines.push(
