#!/usr/bin/env node
// amw-sound-analyze.mjs — Direct-port of dannyjpwilliams/ui-sound-design-skill
// tools/analyze-sound.mjs (MIT, © 2026 Danny Williams). Adapted for the
// ai-maestro-webdesign plugin: invocation path is `node bin/amw-sound-analyze.mjs <file>`
// instead of `node tools/analyze-sound.mjs <file>`. Behavior is otherwise verbatim.
//
// Usage: node bin/amw-sound-analyze.mjs path/to/sound.wav
// Output: JSON profile (duration, envelope, spectral content, tonality,
//   synthesis_suggestion, recipe_starting_point, vocabulary_terms[]).
// Consumed by: agents/amw-sound-designer-agent.md (T-026),
//   skills/amw-ui-sound-design/ (T-001).
//
// MIT License — see LICENSE in plugin root and upstream attribution above.

/**
 * amw-sound-analyze.mjs — Zero-dependency audio file analyzer for the ui-sound-design skill.
 *
 * Extracts a structured sound profile from .wav/.mp3 files that Claude can use
 * to drive Web Audio API synthesis. Run with:
 *
 *   node bin/amw-sound-analyze.mjs <path-to-audio-file>
 *
...
if (!filePath) {
  console.error("Usage: node bin/amw-sound-analyze.mjs <audio-file> [--json]");
...
function decodeWithFfmpeg(path) {
  ...
  const result = execFileSync(
    ...
    { stdio: ["ignore", "pipe", "ignore"], maxBuffer: 100 * 1024 * 1024 }
  );
