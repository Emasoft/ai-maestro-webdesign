...
against a working copy at 1.5.0. [^3]
...
[^3]: [id:ATOM-8Q14-SP7C, status:valid, keywords:"gh_api_contents_returned_an_old_version workflow_says_success_but_the_file_did_not_change is_propagation_broken stale_contents_api_read marketplace_still_shows_the_previous_version", ocd:2026-08-08, lmd:2026-08-08] DO NOT conclude "the update did not land" from a `gh api .../contents/<path>` read, BECAUSE that endpoint can serve a STALE cached blob — measured 2026-08-08: the Notify-Marketplace workflow was green and `marketplace.json` still reported the previous version, while the commit (`1550ace5`, "bump ai-maestro-webdesign to 0.1.11") had already landed and a direct `git/blobs/<sha>` read returned the new version. DO confirm against the commit list for that path, or read the blob by sha, BEFORE reporting a propagation failure — a green workflow plus a stale read looks exactly like a broken publish, and filing that wastes the other project's time on a defect that does not exist.
