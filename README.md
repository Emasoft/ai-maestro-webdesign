...
- `external/hyperframes/` — vendored git submodule; **hyperframes' own code is Apache-2.0** (see `external/hyperframes/LICENSE`). **Transitive-dependency disclosure:** its `@hyperframes/player` package depends on **GSAP** (`gsap@3.15.0` — free *core* only; **no** paid `@gsap/*` club plugins), because a hyperframes composition *is* a paused GSAP timeline. GSAP `3.15.0` ships under the **GreenSock Standard "No-Charge" license** (<https://gsap.com/standard-license>; the package declares this verbatim in its `license` field and bundles no separate LICENSE file) — free of charge and source-available, but **not** an OSI-approved open-source license like MIT/Apache. GSAP appears **only** in the HTML→MP4 video-render path; webpage output follows the plugin's **no-GSAP rule** (native / `animations.html` / Popmotion stack), the bridge adds no GSAP dependency of its own, and the rendered MP4 contains no GSAP — it's encoded pixels, not a GSAP redistribution. **Decision (settled, accepted):** per the Standard License, **GSAP commercial use is free with no fee triggers** — a commercial advert/video produced via hyperframes owes GSAP nothing, and even the formerly members-only plugins are now free for commercial use. The only residual is that GSAP's license is free-but-not-OSI; the no-GSAP-in-website-code rule stays in force regardless. Full record: `design/tasks/TRDD-20260531_221948+0200-e4d97761-gsap-hyperframes-license-decision.md`.
