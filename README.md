...
- `external/hyperframes/` — vendored git submodule; **hyperframes' own code is Apache-2.0** (see `external/hyperframes/LICENSE`). **Transitive-dependency disclosure:** its `@hyperframes/player` package depends on **GSAP** (`gsap@3.15.0` — free *core* only; **no** paid `@gsap/*` club plugins), because a hyperframes composition *is* a paused GSAP timeline. GSAP `3.15.0` ships under the **GreenSock Standard "No-Charge" license** (<https://gsap.com/standard-license>; the package declares this verbatim in its `license` field and bundles no separate LICENSE file) — free of charge and source-available, but **not** an OSI-approved open-source license like MIT/Apache. GSAP appears **only** in the HTML→MP4 video path; webpage output still follows the plugin's no-GSAP rule, and the bridge itself adds no GSAP dependency. Whether this free-but-non-OSI dependency is acceptable for shipping is a **pre-publish licensing decision** (see below).
