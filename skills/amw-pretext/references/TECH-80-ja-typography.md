...
- [How it works — Decision Rules](#how-it-works--decision-rules)
- [Phrase-break × Alignment Matrix](#phrase-break--alignment-matrix)
- [Text Alignment](#text-alignment)
- [Browser Compatibility](#browser-compatibility)
- [Responsive Verification](#responsive-verification)
- [Tailwind Pattern](#tailwind-pattern)
- [Table Cells](#table-cells)
- [Manual Phrase Protection](#manual-phrase-protection)
- [Data-Controlled Line Breaks for Card Templates](#data-controlled-line-breaks-for-card-templates)
- [Card Density Budget](#card-density-budget)
- [Coverage Tiers: CSS baseline → BudouX](#coverage-tiers-css-baseline--budoux)
- [CMS & Template-Driven Content](#cms--template-driven-content)
...
Applies production-grade Japanese web typography to HTML/CSS, Tailwind, React, Next.js, Vue, Svelte, and Astro. Provides:

- A full CSS baseline: font stack, letter-spacing, line-height, CJK line-break rules (`line-break: strict`, `word-break: auto-phrase`), and text-align defaults.
- A phrase-aware breaking system coupled to text-align and content length.
- Explicit suppression of `text-align: justify` on Japanese mobile cards — the single most common Japanese mobile typography bug.
- BudouX `<wbr>` insertion for WebKit (all iOS browsers) and Gecko (Firefox), where `auto-phrase` does not run.
- Quoted-phrase protection (`「〜」` → `<span class="nowrap">`) against BudouX mid-quote splits.
- Card-density budget formula, responsive verification breakpoints, and a per-structure layout-check checklist.

## When to use

- A Japanese page has awkward line breaks: particles strand at line ends, headings look cramped, or the text wraps mid-word on iPhone.
- A mobile card layout has visible gaps between characters (inter-character spacing from `text-align: justify`).
- A heading with a quoted phrase (`「なぜ処方するのか」`) breaks inside the quotation marks.
- Initiating a new Japanese site or section: apply the CSS baseline (Step 0 scope agreement) before any decorative work.
- Escalating from CSS-only fixes to BudouX for full WebKit/Gecko phrase-break coverage.
- CMS/template-builder sites where the developer cannot hand-tune every break.

## When NOT to use

- The content is not Japanese. Use TECH-32 (multilingual/bidi) for general CJK or mixed scripts.
- The issue is font-measurement accuracy (pretext's domain) — use TECH-16 (CJK keep-all) or TECH-11 (setLocale) instead.
- The request is pure font selection or visual style ("make the type nicer") — route to `amw-design-principles/typography-system.md`.

## How it works — Baseline CSS

### Step 0 — Agree the scope before touching code

Japanese typography work ranges from a 5-minute CSS baseline to a build-pipeline change. Applying everything unasked surprises the user and usually over-reaches. **Before editing any code, present the scope as a short multiple-choice question and let the user pick.** Explain each option in plain terms — "pick this → this is what changes."
...
When entering a project, present alignment AND phrase-break decisions as multiple-choice scope items. Example phrasings in Japanese for end-user clarity.
...
- **⚠️ The #1 JP mobile bug — `text-align: justify` on card descriptions:** On a 13-character-wide mobile card column, `justify` inserts visible inter-character gaps on lines 1..N-1 and leaves an empty right edge on the last line. If you see this pattern, check Rule 20 and the "Text Alignment" section immediately.
- **`auto-phrase` is Blink-only** — it does not run on any iPhone (WebKit), any iPad Safari, or Firefox. Always write `word-break: normal` as the fallback line above `auto-phrase`, and escalate to BudouX (Tier 2) if the iOS/Firefox residual is unacceptable.
- **BudouX + left-aligned body = right-edge gaps** — applying BudouX to Tier C surfaces (left-aligned card descriptions, FAQ answers, article body) produces visible right-edge gaps that look like a CSS bug. This is a Rule 21 violation. Always confirm alignment before applying BudouX.
- **`<wbr>` does nothing without `word-break: keep-all`** on the containing element (for form (a) of BudouX). The `<wbr>` break opportunity is only honoured after the existing break-between-any-two-chars default is suppressed.
- **Quoted phrases (`「〜」`) must be nowrapped before BudouX runs** — BudouX treats the brackets as segment boundaries and will split inside the phrase. Pre-process with the `protectQuotedPhrases()` function in the "Coverage Tiers" section.
- **BudouX on card grids requires a per-card layout pass** — inserting `<wbr>` changes where lines break; a card grid can become uneven. Never treat BudouX as "apply and forget" on repeated card layouts.
- **Long proper nouns overflow with blanket `.nowrap`** — use `<wbr>` at safe internal break points for names that may be longer than a card's content width on mobile.
- **`text-wrap: balance` causes blank space on repeated cards** — use `text-wrap: pretty` on card titles / descriptions, not `balance`. Reserve `balance` for unique hero / section headings.
- **Synthetic bold crushes CJK glyphs** — always set an explicit `font-weight` that matches an available weight in the font file.

## Cross-references

- [TECH-16-cjk-keep-all](TECH-16-cjk-keep-all.md) — CJK `word-break: keep-all` (pretext measurement context; complements BudouX)
- [TECH-32-multilingual-bidi](TECH-32-multilingual-bidi.md) — general multilingual / bidi / emoji measurement
- [TECH-81-zh-typography](TECH-81-zh-typography.md) — sibling ZH reference (placeholder stub)
- [_index](_index.md) — full pretext TECH catalog
- `skills/amw-design-principles/typography-system.md` — broader plugin typography rules this TECH file extends

---

*Source: yusuke-hanaue/ja-webtypo-skill, MIT (© 2026 Yusuke Hanaue / 花上雄介). Direct-port preserved verbatim — see attribution comment at top of file for adaptation notes. Original repo: ja-webtypo-skill (search upstream identity if exact GitHub URL needed).*
