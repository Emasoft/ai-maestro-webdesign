...
- `./conversion-matrix.md` — PNG-input row = all `impossible`
- `./validation-dispatcher.md` — PNG branch = fixed-message refusal
...
Wrapper: `../../bin/amw-svg-render.py render <file>` rasterizes to PNG in the plugin state dir. `finish <file>` finalizes and refuses if `render` was never called (see `./svg.md` §5).
...
In `./conversion-matrix.md`, every PNG-row cell is `impossible`. The dispatcher:
