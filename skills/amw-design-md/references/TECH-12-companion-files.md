...
- [Additional companion: `design-showcase.html` (visual QA)](#additional-companion-design-showcasehtml-visual-qa)
...
## Additional companion: `design-showcase.html` (visual QA)

In addition to the four canonical companions emitted by `bin/amw-design-md-emit-companions.py`, the skill ships a sibling generator that produces a **single-file HTML showcase**:

```bash
python3 bin/amw-design-md-showcase.py <DESIGN.md> -o design-showcase.html
```

The showcase is an author-side / reviewer-side visual-QA artifact. It is NOT a build-system consumable (use `tokens.css` / `tokens.json` for that) and it is NOT generated by `amw-design-md-emit-companions.py` (it is a separate bin script so it can be re-rendered on its own without touching the four canonical companions).

What it renders, section by section:

| Section | Contents | Source field |
|---|---|---|
| Colors | Swatch grid grouped (Brand / Foreground / Surface / Text / Borders / Semantic) with a contrast-pairs table for every `X` / `on-X` semantic pair. Every chip has `role="img"` + `aria-label`. WCAG-AA / AAA / AA-large / FAIL badges per pair. | `colors:` frontmatter |
| Typography | One row per role, mono-spec on the left, live specimen on the right (font-family resolved via system-font fallbacks so the file is offline-safe). Label / eyebrow roles render uppercase. | `typography:` frontmatter |
| Rounded | Visible chip for each border-radius step. | `rounded:` frontmatter |
| Spacing | Bar of the declared width per step. | `spacing:` frontmatter |
| Elevation | Shadowed card for each elevation step. Optional — only emitted if `elevation:` or `shadows:` declared. | `elevation:` / `shadows:` frontmatter |
| Components | Rendered button / input / card / chip example for each declared component, with the raw token spec next to it. Token references like `{colors.primary}` are resolved in the demo but preserved verbatim in the spec table. | `components:` frontmatter |

Hard rules for the showcase:

- Single-file HTML. Inline CSS only. No external `<link>`, no `<script>`, no remote fonts.
- Pure Python stdlib (pyyaml optional — falls back to an indentation-based parser).
- ASCII-only output (no CJK). System-font fallbacks throughout.
- Every color swatch carries `aria-label` (chip role="img") AND a visible WCAG badge.
- File is overwritten on re-render — same synchronization rule as the four canonical companions.

When to (re-)generate the showcase:

- After authoring a new DESIGN.md, before delivery — sanity-check that every token actually renders.
- After editing tokens — the showcase is a 1-second visual diff: re-render, double-click, scan.
- As part of an audit (`amw-design-md-auditor-agent` Pass 4 — completeness).
