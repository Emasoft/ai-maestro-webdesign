...
<!-- Adapted for ai-maestro-webdesign plugin: `tools/analyze-sound.mjs` rewritten to `../../../bin/amw-sound-analyze.mjs`; `references/audio-rules.md` rewritten to `references/web-audio-safety.md`; `references/web-audio-api.md` rewritten to `references/web-audio-building-blocks.md`. -->
...
The user ran `node bin/amw-sound-analyze.mjs reference.wav` on an audio file they want to match. The output contains:
